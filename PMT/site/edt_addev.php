...
if(isset($_POST["submit"]))
{
	if($nbdev > 0)
	{
		if($generique == "on")
		{
			$update  = "update ctn_items set _texte = '".addslashes($texte_ckeditor)."' where _IDcours = $IDx and _nosemaine = $nosemaine and _type = 1";
		}
		else
		{
			$update  = "update ctn_items set _texte = '".addslashes($texte_ckeditor)."' where _IDcours = $IDx and _type = 1";
		}
	...
	else
	{
		if($generique == "on")
		{
			$update  = "insert into ctn_items values ('', '_IDctn', '$IDmat', '_ID' , '_IP', '', '1:00', 'chap', '".addslashes($texte_ckeditor)."', 'N', '', 'O', '1', '$IDx', '$nosemaine')";
		}
		else
		{
			$update  = "insert into ctn_items values ('', '_IDctn', '$IDmat', '_ID' , '_IP', '', '1:00', 'chap', '".addslashes($texte_ckeditor)."', 'N', '', 'O', '1', '$IDx', '$nosemaine')";
