    ...
    <table class="width100">
      ...
      <tr>
        <td></td>
        <td colspan="3">
          Re-calculer les numéros de promotion en fonction de l'année d'obtention du diplôme et du nombre d'années du cursus:<br>
          <span class="btn btn-danger" id="reassignPromotionByGraduationYear" title="En fonction de l'année d'obtention du diplôme et du nombre d'années du cursus">Re-calculer les numéros de promotion</span>
        </td>
      </tr>
  ...
  <?php
    $paramList = stripslashes(trim(getParam('paramGestionList')));
  ?>
        ...
        <table class="width100">
          ...
          while ($row = mysqli_fetch_array($result, MYSQLI_NUM)) {
            if (strpos($paramList, "\"".$row[0]."\"") === false)
            {
              ...
              if (substr($row[1], 0, 1) == "{") echo "<textarea class=\"form-control\" rows=\"5\" id=\"input_value_".addslashes($row[0])."\" name=\"input_value_".addslashes($row[0])."\" style=\"width: 90%\">".$row[1]."</textarea>";
