...
/*
 *		module   : download_ged_admin.htm
 *		projet   : la page de téléchargement de la ged administrative d'un élève
 *
 *		version  : 1.0
 *		auteur   : Thomas Dazy
 *		creation : 17/10/2019
...
if ($_SESSION['CnxAdm'] == 255)
{
  ...
  if ($zip->open('tmp/'.$userID.'.zip', ZipArchive::CREATE) === TRUE)
  {
    ...
    while ($row = mysqli_fetch_array($result, MYSQLI_NUM)) {
      ...
      $zip->addFile('download/user_ged/files/'.$userID.'/'.$row[0].'.'.$row[2], $compteur."_".$row[1]);
      $compteur++;
    }
    $zip->close();
    $file = 'tmp/'.$userID.'.zip';
    $fileName = getUserNameByID($userID).'_ged_admin.zip';
